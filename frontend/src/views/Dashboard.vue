...
const formatJobScheduleDisplay = (job: ScheduledJob) => {
  const formatted = formatShortDateTime(job.next_run_time, true)
  if (job.execution_mode === 'range' && job.range_start && job.range_end) {
    const parts = formatted.split(' ')
    const datePart = parts[0] || ''
    return datePart ? `${datePart} ${job.range_start}~${job.range_end}` : `${job.range_start}~${job.range_end}`
  }
  return formatted
}
