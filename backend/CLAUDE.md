...
A: `tasks.py` + `POST /batch/tasks` 使用旧版 SQLAlchemy ORM，已标记弃用且**默认只读**（`APP_LEGACY_TASKS_READONLY=1`）；`sign_tasks_v2.py` + `POST /batch/sign-tasks` 使用文件存储体系。新功能必须使用 sign-tasks。存量自查：`GET /api/tasks/legacy-status`。
