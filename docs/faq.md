...
- 只想搬任务流程 → 用 **JSON**  
- 换服务器整机恢复 → 用 **完整备份**（可上传 WebDAV 或对象存储），停止服务后解压到 data 目录再启动（见 [备份与恢复](guide/backup-webdav.md)、[运维手册](reference/ops.md)）

## 如何配置 WebDAV 自动备份？

见 [备份与恢复](guide/backup-webdav.md)：设置 → 完整备份 → 填 URL/账号（或对象存储连接信息）→ 测试连接 → 开启自动备份并保存。上传成功会清理本地副本并按保留份数轮转远端；失败会尽量发 Bot 通知。
