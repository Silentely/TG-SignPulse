...
3. **主服务防雪崩隔离**：每个插件在独立沙箱中加载执行，单个插件发生语法错误或缺少三方包绝不导致主服务或定时调度器崩溃。
4. **防死锁与超时熔断**：插件执行默认及显式配置超时熔断（`timeout`），超时即安全释放任务并切断等待，避免永久占用账号 MTProto 会话锁。
...
系统在启动或执行时会自动扫描并动态挂载以下目录下的插件（按优先级生效）：
...
> **Docker 挂载提示**：
> 在 `docker-compose.yml` 默认挂载 `./data:/data` 的情况下，只需在宿主机项目根目录创建 `./data/plugins/你的插件/main.py`，启动容器即可自动识别，**无需构建任何新镜像**！
