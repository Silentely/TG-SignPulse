...
2. **系统设置中的插件概览、重新加载与调试演练场 (Playground)**：
   ...
   - **调试演练场 (Playground)**：点击插件右侧的 **「演练」** 按钮，可弹出进程内测试窗口。直接输入一段模拟的 Bot 消息文本（如 `请在 30 秒内输入 2*31 的答案` 或 `验证码为：9527`），点击 **「执行测试」** 即可实时看到插件的匹配结果、提取回复内容、详细日志流与执行耗时（毫秒级），无需等待实际打卡或触发账号签到。插件代码拥有宿主进程权限，请只测试可信插件；超时不能强制终止已经运行的同步线程；
...
| `/api/plugins/{name}/test` | `POST` | Web 进程内调试接口：模拟传入消息文本和参数，返回匹配结果、回复内容、日志与耗时。仅用于可信插件。 |
