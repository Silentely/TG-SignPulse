...
1. **自动候选与任务编排**：
   ...
   - 在动作下拉菜单中选择 **自定义插件 (99)**；
   - 在右侧插件标识输入框中，系统会自动联想下拉候选（列出已加载的插件名及其运行模式，如 `math_solver (reactive)`）；选中后还会以高亮标签显示插件描述；同时依然支持自由输入任意合法插件名；
   - 保存任务配置。当签到任务发送 `/checkin` 并收到 Bot 发出的算式验证码时，系统将触发 `math_solver` 本地秒答并自动回复计算结果！

2. **系统设置中的插件概览与热重载**：
   - 进入 **系统设置** 页面，在左侧找到 **扩展插件** 卡片；
   - 卡片中完整列出当前所有已挂载插件的名称、运行模式（`reactive` 响应监听 / `active` 主动执行）、描述及源文件路径；
   - 放入新的插件文件或修改现有代码后，**无需重启服务或容器**，只需点击右上角的 **「重新加载」** 按钮，即可瞬间完成全量热扫描并更新可用列表。

### 插件管理 API

| 接口 | 方法 | 说明 |
| :--- | :--- | :--- |
| `/api/plugins` | `GET` | 获取当前所有已加载插件的元数据列表（名称、模式、描述、源码路径）。 |
