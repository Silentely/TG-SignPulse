...
通过本目录，你可以直接提交 Pull Request（PR）向官方仓库贡献自定义 Action 插件。一旦 PR 审查合并入主分支，系统流水线将自动完成语法校验、安全审计、索引构建与打包分发，所有 TG-SignPulse 运行实例即可在 Web 管理界面的「插件市场」中一键拉取安装与热重载。
...
7. **提交 PR**：
   向官方仓库的 `main` 或 `dev` 分支提交 Pull Request，等待 CI 自动化安全门禁检查与审核合并。
