...
- 通知与状态：支持 Telegram Bot 失败通知和关键词命中通知；账号下所有任务均失败时首页会提示登录失效
...
- 在有序动作序列中添加 `关键词监听`，命中后可推送到 Telegram Bot、Bark、自定义 URL，或转发到指定 Chat ID/话题 ID
...
- **自定义推送渠道**：关键词命中后可继续使用 Telegram Bot、Bark 或自定义推送 URL；转发失败不会中断后续推送。
- **关键词监听布局优化**：关键词、匹配模式、忽略大小写、转发 Chat ID、话题 ID、推送渠道和推送 URL 均改为一行一个参数，避免配置项挤在一起。
